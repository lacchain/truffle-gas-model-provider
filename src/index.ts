...
class GasModelProvider {
    ...
    constructor(...args: ConstructorArguments) {
        ...
        let providerToUse;
        if (GasModelProvider.isValidProvider(provider)) {
            providerToUse = provider;
        } else if (GasModelProvider.isValidProvider(url)) {
            providerToUse = url;
        } else {
            providerToUse = providerOrUrl;
        }

        if (!GasModelProvider.isValidProvider(providerToUse)) {
            throw new Error(
                [
        ...
        this.engine.addProvider(
            new HookedSubprovider({
                ...
                async signTransaction(txParams: any, cb: any) {
                    ...
                    const web3 = new Web3();
                    const extra = web3.eth.abi.encodeParameters(
                        ["address","uint256"],
                        [nodeAddress, expiration]);
                    const rawTx = `0x${signedTx.serialize().toString("hex")}${extra.substring(2)}`;
                    cb(null, rawTx);
                },
        ...
        if (typeof providerToUse === "string") {
            ...
            switch (providerProtocol) {
                ...
                case "wss:":
                    ...
                    break;
                default:
                    const rpc = new RpcProvider({rpcUrl: url});
                    this.engine.addProvider(rpc);
            }
        } else {
